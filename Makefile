...
HOME_URL ?= http://hellorust.com/booster
...
# change it.
UPLOAD_PATH ?= rediger:/var/www/sites/www.hellorust.com/booster/
...
upload: html
	rsync -avr html/*html $(UPLOAD_PATH)

clean:
	rm -f html/*.html
	rm -f src/*.exe

html: html/index.html html/hint-mutable-borrow-1.html html/hint-struct-1.html html/install.html

html/%.html: md/%.md sync-index.py $(wildcard src/*rs)
	mkdir -p html
	markdown $< | python2 sync-index.py "$(HOME_URL)" > $@
